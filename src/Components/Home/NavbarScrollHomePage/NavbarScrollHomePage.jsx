...
export default function NavbarScrollHomePage({
  ...
  // ✅ draggable offset (applied via transform)
  ...
  /* ---------------------------------------
     ...
     - writes CSS vars --dx/--dy (transform)
  ...
  const clampToViewport = useCallback((x, y) => {
    ...
    // rect already includes transform; clamp offsets relative to current rect position
  ...
  const onHandlePointerDown = useCallback(
    (e) => {
      if (!visible) return; // only drag when visible
      const nav = navRef.current;
      if (!nav) return;
  ...
  const onHandlePointerUp = useCallback((e) => {
    ...
    try {
      localStorage.setItem(
        LS_POS,
        JSON.stringify({ x: dragRef.current.x, y: dragRef.current.y })
      );
    } catch {}
  }, []);

  // safety: stop dragging on blur/escape
  useEffect(() => {
    const onBlur = () => {
...
//     const rect = nav.getBoundingClientRect();
//     // base position already applied; we just constrain offsets so the card stays on screen
//     const pad = 10;
//     const maxRight = window.innerWidth - pad - rect.width;
//     const maxBottom = window.innerHeight - pad - rect.height;

//     // current rect.left/top already includes transform, so we estimate allowed offsets by comparing
//     // We do a safe clamp around viewport using current rect:
//     const dxMin = -rect.left + pad;
//     const dxMax = maxRight - rect.left;
//     const dyMin = -rect.top + pad;
//     const dyMax = maxBottom - rect.top;

//     return { x: clamp(x, x + dxMin, x + dxMax), y: clamp(y, y + dyMin, y + dyMax) };
//   }, []);
...
//   const onHandlePointerDown = useCallback(
//     (e) => {
//       if (!visible) return; // only drag when actually visible
//       const nav = navRef.current;
//       if (!nav) return;
...
//     // final commit already done via raf; ensure saved:
//     try {
...
//         {/* ✅ bottom row: drag hint + top button */}
//         <div className="navbar__scroll__hp__bottomRow">
//           <button
//             type="button"
//             className="navbar__scroll__hp__dragHandle"
...
//           >
//             <span className="navbar__scroll__hp__dragDots" aria-hidden="true" />
//             <span className="navbar__scroll__hp__dragTxt">déplace-moi</span>
//           </button>
...
//             title="Back to top"
//           >
//             <span className="navbar__scroll__hp__topIcon" aria-hidden="true">↑</span>
//           </button>
//         </div>
//       </div>
//     </nav>
