...
export default function FullScreenLoader({
  ...
  // ✅ MONOTONE REAL PROGRESS:
  // - évite le flash à 0% quand useProgress redémarre (collision GLB)
  // - n'utilise PAS "||" (car 0 est un vrai nombre)
  const lastMaxRealRef = useRef(0);

  const pctReal = useMemo(() => {
    const hasPhase = Number.isFinite(phases.overallPct);
    const base = hasPhase ? phases.overallPct : pctGlobal;

    const clamped = Math.min(100, Math.max(0, Math.round(base)));
    lastMaxRealRef.current = Math.max(lastMaxRealRef.current, clamped);

    return lastMaxRealRef.current;
  ...
  /* -------------------------------------------
     ...
     - ⚠️ NE RESET PAS sur active=false (car flicker possible entre loads)
  -------------------------------------------- */
  const [pctDisplay, setPctDisplay] = useState(0);

  useEffect(() => {
    // ✅ IMPORTANT:
    // useProgress.active peut flicker false 1 frame entre deux loads
    // → on garde le display (pas de reset)
    if (!active) return;
